...
// ── Middleware: admin o auditor ──
function adminOAuditor(req, res, next) {
  if (!['admin','auditor'].includes(req.usuario.rol))
    return res.status(403).json({ error: 'Sin permisos para registrar movimientos' });
  next();
}
...
router.get('/', auth, async (req, res) => {
  try {
    ...
    // Supervisor solo ve movimientos de su área
...
// POST /api/asignaciones - solo admin y auditor
...
// POST /api/asignaciones/masivo - solo admin y auditor
router.post('/masivo', auth, adminOAuditor, upload.single('archivo'), async (req, res) => {
  try {
    ...
    for (const fila of filas) {
      ...
      try {
        const eqResult = await pool.query('SELECT id FROM equipos WHERE imei1=$1 OR imei2=$1 OR numero_serie=$1 LIMIT 1', [imei]);
        ...
        if (rut && tipo === 'asignacion') {
          const fResult = await pool.query('SELECT id FROM funcionarios WHERE rut ILIKE $1 AND activo=true LIMIT 1', [rut]);
          if (!fResult.rows.length) { resultados.errores.push(`IMEI ${imei}: RUT ${rut} no encontrado`); continue; }
          funcionarioId = fResult.rows[0].id;
        }
        if (estadoMap[tipo]) await pool.query('UPDATE equipos SET estado=$1, updated_at=NOW() WHERE id=$2', [estadoMap[tipo], equipoId]);
        await pool.query(`INSERT INTO asignaciones (equipo_id, funcionario_id, usuario_id, tipo, observacion) VALUES ($1,$2,$3,$4,$5)`,
...
// GET /api/asignaciones/plantilla-masivo
router.get('/plantilla-masivo', auth, adminOAuditor, (req, res) => {
...
// GET /api/asignaciones/export/excel - solo admin y auditor
router.get('/export/excel', auth, adminOAuditor, async (req, res) => {
  try {
